...
public class OrderServiceTests
{
    ...
    {
        using var context = CreateContext();
        var fakeItem = new InventoryItem { Id = 1, ProductId = 1, ProductName = "Widget A", QuantityOnHand = 45 };
    ...
    {
        using var context = CreateContext();
        var errorBody = new { error = "Insufficient stock for product 1. Available: 50, Requested: 99999" };
        var inventoryClient = CreateMockInventoryClient(HttpStatusCode.Conflict, errorBody);
        var service = new OrderService(context, inventoryClient);
        var product = await context.Products.FirstAsync();
        var customer = await context.Customers.FirstAsync();
