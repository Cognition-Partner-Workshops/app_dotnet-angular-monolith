...
internal class FakeInventoryHandler : HttpMessageHandler
{
    ...
    protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        if (path.Contains("/deduct"))
        {
            ...
            var json = JsonSerializer.Serialize(new { id = 1, productId = 1, productName = "Test", quantityOnHand = 45, reorderLevel = 10, warehouseLocation = "A-01", lastRestocked = DateTime.UtcNow });
            return Task.FromResult(new HttpResponseMessage(HttpStatusCode.OK)
            {
                Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
            });
