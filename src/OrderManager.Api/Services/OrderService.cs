...
public class OrderService
{
    ...
    public async Task<Order> CreateOrderAsync(int customerId, List<(int ProductId, int Quantity)> items)
    {
        ...
        foreach (var (productId, quantity) in items)
        {
            ...
            var stockResult = await _inventoryClient.CheckAndDeductStockAsync(productId, quantity);
            if (!stockResult.Success)
                throw new InvalidOperationException(stockResult.Error ?? $"Insufficient stock for {product.Name}");
