...
public class OrderService
{
    ...
    public async Task<Order> CreateOrderAsync(int customerId, List<(int ProductId, int Quantity)> items)
    {
        ...
        foreach (var (productId, quantity) in items)
        {
            ...
            var stock = await _inventoryClient.GetStockQuantityAsync(productId);
            if (stock < quantity)
                throw new InvalidOperationException($"Insufficient stock for {product.Name}. Available: {stock}");
