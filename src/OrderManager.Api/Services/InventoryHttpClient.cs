...
public class InventoryHttpClient
{
    ...
    public async Task<InventoryItemDto> DecrementStockAsync(int productId, int quantity)
    {
        var response = await _httpClient.PostAsJsonAsync($"/api/inventory/product/{productId}/decrement", new { Quantity = quantity });
        ...
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<InventoryItemDto>()
            ?? throw new InvalidOperationException("Failed to deserialize decrement response");
    }
}
